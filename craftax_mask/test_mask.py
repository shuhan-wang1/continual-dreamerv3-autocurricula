...
def test_rules():
    ...
    assert isinstance(ACTION_PLACE_TABLE, int)
    assert isinstance(ACTION_MAKE_WOOD_PICKAXE, int)
    assert BASIC_ACTION_RULES["PLACE_TABLE"]["requires"] == {"wood": 2}
    assert BASIC_ACTION_RULES["MAKE_WOOD_PICKAXE"]["needs_table"] is True
    assert BASIC_ACTION_RULES["PLACE_TABLE"]["action_id"] == ACTION_PLACE_TABLE
    assert BASIC_ACTION_RULES["MAKE_WOOD_PICKAXE"]["action_id"] == ACTION_MAKE_WOOD_PICKAXE
    print("rules: OK")


def test_mask_soft():
    from .mask import assert_basic_mask_expectations, compute_logit_bias, compute_mask_details, compute_mask_logging_stats
    ...
    # missing stone -> MAKE_STONE_PICKAXE (12) penalized
    ctx4 = {"wood": 3, "stone": 0, "near_table": True, "near_furnace": True}
    bias4 = compute_logit_bias(ctx4, BASIC_ACTION_RULES, num_actions=43, mode="soft")
    assert bias4[12] < 0, "MAKE_STONE_PICKAXE should be penalized when stone is missing"
    print("mask soft (missing stone, MAKE_STONE_PICKAXE suppressed): OK")

    details = compute_mask_details(ctx3, BASIC_ACTION_RULES, num_actions=43, mode="soft")
    assert_basic_mask_expectations(ctx3, details)
    stats = compute_mask_logging_stats(
        details,
        BASIC_ACTION_RULES,
        raw_logits=np.zeros(43, dtype=np.float32),
        adjusted_logits=np.asarray(bias3, dtype=np.float32),
    )
    assert stats["mask/invalid_make_wood_pickaxe_count"] == 1.0
    assert stats["mask/invalid_make_stone_pickaxe_count"] == 1.0
    print("mask soft diagnostics: OK")
