...
# ──────────────────────────────────────────────────────────────────────
# CSV Exporter  (N-method compatible)
# ──────────────────────────────────────────────────────────────────────

class CSVExporter:
    """Accumulates numerical data from all figures and writes CSV files.

    All methods are stored by their display name so the CSVs are
    self-describing regardless of how many experiment methods exist.
    """

    def __init__(self):
        self.timeseries_rows = []       # curves_timeseries.csv
        self.checkpoint_rows = []       # checkpoint_statistics.csv
        self.ach_rate_rows = []         # achievement_rates_bar.csv
        self.ach_forg_rows = []         # achievement_forgetting_bar.csv
        self.score_dist_rows = []       # score_distribution.csv
        self.heatmap_rows = []          # achievement_heatmap.csv
        self.summary_rows = []          # summary_table.csv

    # --- accumulation helpers ---

    def add_timeseries(self, figure, metric, method, grid, matrix):
        """Store mean/std time-series for one method."""
        mu = matrix.mean(axis=0)
        sd = matrix.std(axis=0)
        for i in range(len(grid)):
            self.timeseries_rows.append({
                "figure": figure, "metric": metric, "method": method,
                "step": grid[i], "mean": mu[i], "std": sd[i],
            })

    def add_timeseries_seed(self, figure, metric, method, seed, steps, values):
        """Store a single-seed trajectory."""
        for i in range(len(steps)):
            self.timeseries_rows.append({
                "figure": figure, "metric": metric,
                "method": f"{method}_seed{seed}",
                "step": steps[i], "mean": values[i], "std": 0.0,
            })
    ...
    def add_checkpoint(self, figure, metric, checkpoint, method_a, vals_a,
        ...
        d = (vals_a.mean() - vals_b.mean()) / pooled if pooled > 1e-8 else 0.0
        self.checkpoint_rows.append({
            "figure": figure, "metric": metric, "checkpoint": checkpoint,
            "method_a": method_a,
            "method_a_mean": vals_a.mean() if len(vals_a) else "",
            "method_a_std": vals_a.std() if len(vals_a) else "",
            "method_b": method_b,
            "method_b_mean": vals_b.mean() if len(vals_b) else "",
            "method_b_std": vals_b.std() if len(vals_b) else "",
            "welch_t": t_stat if not np.isnan(t_stat) else "",
            "p_value": p_val if not np.isnan(p_val) else "",
            "significance": sig, "cohens_d": d,
        })

    def add_achievement_rate(self, achievement, method, mean, std):
        self.ach_rate_rows.append({
            "achievement": achievement, "method": method,
            "mean": mean, "std": std,
        })

    def add_achievement_forgetting(self, achievement, method, mean, std):
        self.ach_forg_rows.append({
            "achievement": achievement, "method": method,
            "mean": mean, "std": std,
        })

    def add_score_distribution(self, bin_label, method, mean, std):
        self.score_dist_rows.append({
            "bin": bin_label, "method": method,
            "mean": mean, "std": std,
        })

    def add_heatmap_row(self, method, step, achievement, mean_rate):
        self.heatmap_rows.append({
            "method": method, "step": step,
            "achievement": achievement, "mean_rate": mean_rate,
        })

    def add_summary(self, metric, method, mean, std):
    ...
    def write_all(self, output_dir):
        d = pathlib.Path(output_dir)
        self._write_csv(d / "curves_timeseries.csv",
            ["figure", "metric", "method", "step", "mean", "std"],
            self.timeseries_rows)
        self._write_csv(d / "checkpoint_statistics.csv",
            ["figure", "metric", "checkpoint",
             "method_a", "method_a_mean", "method_a_std",
             "method_b", "method_b_mean", "method_b_std",
             "welch_t", "p_value", "significance", "cohens_d"],
            self.checkpoint_rows)
        self._write_csv(d / "achievement_rates_bar.csv",
            ["achievement", "method", "mean", "std"],
            self.ach_rate_rows)
        self._write_csv(d / "achievement_forgetting_bar.csv",
            ["achievement", "method", "mean", "std"],
            self.ach_forg_rows)
        self._write_csv(d / "score_distribution.csv",
            ["bin", "method", "mean", "std"],
            self.score_dist_rows)
        self._write_csv(d / "achievement_heatmap.csv",
            ["method", "step", "achievement", "mean_rate"],
            self.heatmap_rows)
        self._write_csv(d / "summary_table.csv",
            ["metric", "method", "mean", "std"],
            self.summary_rows)
    ...
    for idx, (value_key, ylabel, title) in enumerate([
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                mats[mk] = mat
                ref_grid = grid
                csv_exporter.add_timeseries("Fig01", value_key, METHODS[mk], grid, mat)
        if ref_grid is not None and len(mats) >= 2:
            add_pairwise_significance(ax, ref_grid, mats)
            # Pairwise checkpoint stats for CSV
            mk_list = list(mats.keys())
            n = ref_grid.shape[0]
            for i_a in range(len(mk_list)):
                for i_b in range(i_a + 1, len(mk_list)):
                    for label, frac in [("25%", 0.25), ("50%", 0.50), ("75%", 0.75), ("100%", 1.0)]:
                        ci = min(int(frac * (n - 1)), n - 1)
                        csv_exporter.add_checkpoint("Fig01", value_key, label,
                            METHODS[mk_list[i_a]], mats[mk_list[i_a]][:, ci],
                            METHODS[mk_list[i_b]], mats[mk_list[i_b]][:, ci])
    ...
    for idx, (value_key, ylabel, title) in enumerate([
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                mats[mk] = mat
                ref_grid = grid
                csv_exporter.add_timeseries("Fig02", value_key, METHODS[mk], grid, mat)
        if ref_grid is not None and len(mats) >= 2:
            add_pairwise_significance(ax, ref_grid, mats)
        format_ax(ax, title, ylabel=ylabel)
    ...
    for idx, (value_key, title) in enumerate(loss_keys):
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                mats[mk] = mat
                ref_grid = grid
                csv_exporter.add_timeseries("Fig03", value_key, METHODS[mk], grid, mat)
        if ref_grid is not None and len(mats) >= 2:
            add_pairwise_significance(ax, ref_grid, mats)
    ...
    for idx, (vk1, vk2, ylabel, title) in enumerate([
        ...
        for value_key in [vk1, vk2]:
            ...
            for mk in METHODS:
                ...
                if grid is not None and mat is not None:
                    plot_mean_std(ax, grid, mat, COLORS[mk], METHODS[mk], method_key=mk)
                    csv_exporter.add_timeseries("Fig04", title, METHODS[mk], grid, mat)
                    any_data = True
            if any_data:
                break
        format_ax(ax, title, ylabel=ylabel)
    ...
    for idx, (value_key, title) in enumerate(explore_keys):
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                ref_grid = grid
                any_data = True
                csv_exporter.add_timeseries("Fig05", value_key, METHODS[mk], grid, mat)
        if not any_data:
    ...
    for idx, (value_key, ylabel, title) in enumerate([
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                mats[mk] = mat
                ref_grid = grid
                csv_exporter.add_timeseries("Fig06", value_key, METHODS[mk], grid, mat)
        if ref_grid is not None and len(mats) >= 2:
            add_pairwise_significance(ax, ref_grid, mats)
        format_ax(ax, title, ylabel=ylabel)
...
def figure_per_achievement_rates(data):
    ...
    for i, mk in enumerate(_method_keys_sorted()):
        ...
        ax.bar(x + offset, mu, width, yerr=std, label=METHODS[mk],
               ...
               linewidth=0.5)
        # CSV: per-achievement rates for this method
        for j in range(n_ach):
            csv_exporter.add_achievement_rate(ACHIEVEMENT_NAMES[j], METHODS[mk], mu[j], std[j])
...
def figure_per_achievement_forgetting(data):
    ...
    for i, mk in enumerate(_method_keys_sorted()):
        ...
        ax.bar(x + offset, mu, width, yerr=std, label=METHODS[mk],
               color=COLORS[mk], alpha=al, capsize=3, edgecolor=ec,
               linewidth=0.5)
        # CSV: per-achievement forgetting for this method
        for j in range(n_ach):
            csv_exporter.add_achievement_forgetting(ACHIEVEMENT_NAMES[j], METHODS[mk], mu[j], std[j])
    ...
    for idx, mk in enumerate(_method_keys_sorted()):
        ...
        if seed_dists:
            ...
            for b in range(len(mu)):
                ...
                csv_exporter.add_score_distribution(lbl, METHODS[mk], mu[b], std[b])
...
def figure_achievement_heatmap_over_time(data):
    ...
    for idx, mk in enumerate(_method_keys_sorted()):
        ...
        # CSV: heatmap data (subsample every 10th step)
        for gi in range(0, n_grid, 10):
            for j in range(len(ACHIEVEMENT_NAMES)):
                csv_exporter.add_heatmap_row(METHODS[mk], grid[gi], ACHIEVEMENT_NAMES[j], mean_rates[j, gi])
        if (n_grid - 1) % 10 != 0:
            for j in range(len(ACHIEVEMENT_NAMES)):
                csv_exporter.add_heatmap_row(METHODS[mk], grid[-1], ACHIEVEMENT_NAMES[j], mean_rates[j, -1])
...
def figure_summary_table(data):
    ...
    for key, label in metrics_to_compare:
        ...
        for mk in method_keys:
            ...
            if len(v) > 0:
                csv_exporter.add_summary(label, METHODS[mk], v.mean(), v.std())
    ...
    for mk in method_keys:
        if mk in unlock_data:
            ...
            csv_exporter.add_summary("Achievements Unlocked (>1%)", METHODS[mk], arr.mean(), arr.std())
        else:
            row.append("N/A")
    ...
    for plot_idx, ach_idx in enumerate(top_indices):
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                mats[mk] = mat
                ref_grid = grid
                csv_exporter.add_timeseries("Fig12", f"achievement/{ach_name}", METHODS[mk], grid, mat)
    ...
    for idx, (value_key, ylabel, title) in enumerate([
        ...
        for mk in METHODS:
            ...
            if grid is not None and mat is not None:
                ...
                mats[mk] = mat
                ref_grid = grid
                csv_exporter.add_timeseries("Fig13", value_key, METHODS[mk], grid, mat)
        format_ax(ax, title, ylabel=ylabel)
    ...
    for idx, mk in enumerate(_method_keys_sorted()):
        ...
        for s_idx, seed in enumerate(SEEDS):
            ...
            if len(s) > 0:
                ...
                csv_exporter.add_timeseries_seed("Fig14", "episode/score", METHODS[mk], seed, s, v)
...
def main():
    ...
    csv_exporter.write_all(OUTPUT_DIR)

    print(f"\nAll figures, stats, and CSV files saved to {OUTPUT_DIR.resolve()}/")
    print("=" * 70)
